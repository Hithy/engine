...
namespace detail {
  ...
  struct get_vector_type<std::vector<T, A>> {
    using type = T;
  };

  template <typename U>
  struct base_box_struct {
    static PyObject* boxing(U obj);
    static U unboxing(PyObject* obj);
  };

  template <typename U>
  struct vector_box_struct {
    ...
    static U unboxing(PyObject* obj) {
      ...
      for (int i = 0; i < list_size; i++) {
        ...
        res[i] = detail::unboxing<get_vector_type<U>::type>(item);
      }
  ...
  template<typename U>
  struct obj_box_struct {
    static PyObject* boxing(U& obj) {
      return PyCXXObject<U>::boxing(obj);
    }

    static U& unboxing(PyObject* obj) {
      return PyCXXObject<U>::unboxing(obj);
    }
  };

  template <typename U>
  struct ptr_box_struct {
    using type_non_ptr = std::remove_pointer_t<U>;

    static PyObject* boxing(U obj) {
      static_assert(std::is_same_v<type_non_ptr, std::remove_pointer_t<type_non_ptr>>);
  ...
  PyObject* boxing(T&& obj) {
    ...
    typedef std::conditional_t<
      ...
      std::conditional_t<
        ...
        std::conditional_t<
          // check vector
          is_std_vector<type_ncrp>::value,
          vector_box_struct<type_ncrp>,
  ...
  template <typename T>
  decltype(auto) unboxing(PyObject* obj) {
    typedef std::remove_reference_t<T> type_base;
    typedef std::remove_const_t<type_base> type_nc;
    typedef std::remove_pointer_t<type_nc> type_ncp;
    typedef std::remove_reference_t<type_nc> type_ncr;
